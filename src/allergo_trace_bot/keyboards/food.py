...
def build_ingredient_search_results(
    ingredients: list[Ingredient], show_add_custom: bool = True
        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    text=f"{ingredient.name} ({ingredient.category})", callback_data=f"ing:{ingredient.id}"
                )
            ]
        )
...
def build_category_ingredients_keyboard(ingredients: list[Ingredient], category: str) -> InlineKeyboardMarkup:
    ...
    for ingredient in ingredients[:20]:
        buttons.append([InlineKeyboardButton(text=ingredient.name, callback_data=f"ing:{ingredient.id}")])
